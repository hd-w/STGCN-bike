...
class STGCNBlock(nn.Module):
    ...
    def __init__(self, in_channels, spatial_channels, out_channels, time_channels, 
                 num_nodes):
        """
        :param in_channels: Number of input features at each node in each time
        step.
        ...
        self.fully_embeding = nn.Linear(out_channels*time_channels,1)
    ...
    def forward(self, X):
        ...
        3. Graph wavenet: A = softmax(ReLu(E1*E2))
        """
        t = self.temporal1(X)#32, 110, 12, 64
        
        emb = self.fully_embeding(t.reshape(-1, t.shape[1], t.shape[2]*t.shape[3]))
        # print(emb.shape)
        emb = torch.mean(emb, 0)
        # print(emb.shape)
        A_hat = F.sigmoid(F.relu(torch.mul(emb.transpose(0,1), emb)))
...
class STGCN(nn.Module):
    ...
    def __init__(self, num_nodes, num_features, num_timesteps_input,
        ...
        self.block1 = STGCNBlock(in_channels=num_features, out_channels=64,
                                 spatial_channels=16, time_channels = 12, num_nodes=num_nodes)
    ...
    def forward(self, X):
        ...
        out1 = self.block1(X) # Time-attention
        out2 = self.block2(out1)# Space-attention
